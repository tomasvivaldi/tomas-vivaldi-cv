...
export const HighlightedProjects = ({
  ...
  return (
    <div
      ref={ref}
      className="h-[150vh] md:h-[250vh] my-8 py-32 overflow-hidden  antialiased relative flex flex-col self-auto [perspective:1000px] [transform-style:preserve-3d]
      "
    >
      <Header />
      <motion.div
  ...
  return (
    <div className="max-w-7xl relative mx-auto py-20 md:py-40 px-4 w-full  left-0 top-0">
      <h1 className="text-4xl md:text-7xl font-bold dark:text-white">
        Beautiful <br /> Software Products
      </h1>
      <p className="max-w-2xl text-base md:text-xl mt-8 dark:text-neutral-200">
        I build beautiful products with the latest technologies and frameworks.
        Passionate developer and designer that love to build amazing products!
      </p>
    </div>
  );
};
...
export const ProductCard = ({
  ...
  return (
    <motion.div
      ...
      key={product.title}
      className="group/product h-60 md:h-96 w-[30rem] relative flex-shrink-0"
    >
      <Link
        href={product.link}
